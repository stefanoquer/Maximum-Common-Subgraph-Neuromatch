...
def print_graph(gnx, anchor, figname):
    labels = dict([(i, f"{i}" + (' A' if anchor == i else ""))
                  for i in gnx.nodes()])

    pos = nx.nx_pydot.graphviz_layout(gnx)

    nx.draw(gnx, cmap="jet", labels=labels, pos=pos)
...
# Supposing graph is connected
def sample_neigh_anchor(graph, n_hops, anchor, available_nodes):
    start_node = anchor
    visited = set([start_node])
    frontier = set(graph.neighbors(start_node)) - set(visited)
    new_frontier = set()
    c = 0
    while c < n_hops:
        c += 1
        for new_node in frontier:
            visited.add(new_node)
            new_frontier = new_frontier.union(
                set(graph.neighbors(new_node)) - visited)
        frontier, new_frontier = new_frontier, set()

    neigh = list(visited.intersection(available_nodes))
    return graph, neigh
...
def batch_nx_graphs(graphs, anchors=None):
    ...
    #datalist = [DSGraph(g) for g in graphs]
    # newGraphs = filter(lambda g: g.number_of_edges() > 0, graphs)
    # print("newg",list(newGraphs))
